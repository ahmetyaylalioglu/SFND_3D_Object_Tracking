...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
    ...
    for (int i = 0; i < kptMatches.size(); i++)
    {
        ...
        {
            boundingBox.kptMatches.emplace_back(kptMatches[i]);
        }
    }
    ...
    for (int i = 0; i < boundingBox.kptMatches.size(); i++)
    {
        ...
        totalDistance = totalDistance + euclideanDist;
        
    }

    meanDist = totalDistance/boundingBox.kptMatches.size();
    ...
    for (int i = 0; i < boundingBox.kptMatches.size(); i++)
    {
        euclideanDist = 0;
        euclideanDist = sqrt(pow((kptsCurr.at(boundingBox.kptMatches[i].trainIdx).pt.x - kptsPrev.at(boundingBox.kptMatches[i].queryIdx).pt.x),2.0) + pow((kptsCurr.at(boundingBox.kptMatches[i].trainIdx).pt.y - kptsPrev.at(boundingBox.kptMatches[i].queryIdx).pt.y),2.0));
        if(euclideanDist > (meanDist * meanRatio))
        {
...
{
   double dt = 1/frameRate; //time which between two lidar measurements in seconds
   double laneWidth = 4; //width of Ego Lane
   ...
   for (auto &it : filteredPrevLidar->points )
    {
    ...
    for (auto &it : filteredCurrLidar->points )
    {
        if((it.y > (-1*laneWidth/2)) && (it.y < (laneWidth/2)))
      {
